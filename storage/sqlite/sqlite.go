...
func (s *Storage) PickRandom(ctx context.Context, userName string) (*storage.Page, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Error selecting page by user: %v", err)
	}
...
func (s *Storage) Remove(ctx context.Context, page *storage.Page) error {
	...
	if _, err := s.db.ExecContext(ctx, q, page.URL, page.UserName); err != nil {
		return fmt.Errorf("Error deleting page: %v", err)
	}
	return nil
}
...
func (s *Storage) IsExists(ctx context.Context, page *storage.Page) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("Error selecting page by user: %v", err)
	}
	return count > 0, nil
}
...
func (s *Storage) Init(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating pages: %v", err)
	}
	return nil
}
